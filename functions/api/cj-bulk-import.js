// Category-driven CJ → Shopify importer using Shopify's Bulk Operations API.
//
// Replaces the 750ms-spaced REST single-create loop with a two-phase pipeline:
//   1. BULK PHASE  — one bulkOperationRunMutation(productCreate) creates every
//                    product (title/vendor/type/tags/description + full option
//                    values) in a single background GraphQL job, bypassing the
//                    REST "2 calls/sec" throttle entirely.
//   2. VARIANT PHASE — productVariantsBulkCreate (regular GraphQL) attaches ALL
//                    variants (price, compare-at, SKU, stock) plus product images
//                    in ONE call per product. This is ~1 call/product (cost ~11
//                    points each) and never touches the throttled REST API.
//
// Progress is persisted to Shopify metafields (NOT GitHub — rate-limits) so
// overlapping/repeated runs are safe and it self-continues. Triggered by GitHub
// Actions cron with ADMIN_PIN.
...
function extractImages(p) {
  const out = [];
  const seen = new Set();
  const push = (u) => { if (u && typeof u === 'string' && !seen.has(u)) { seen.add(u); out.push(u); } };
...
// ── Map CJ detail → { productInput, variants, images } for the 2 phases ──
function buildBulkProduct(pid, d) {
  ...
  // Full option values (dedupe, preserve order) for productOptions.
  const optionValues = optionNames.map(() => new Set());
  const gqlVariants = variants.map(v => {
    const parts = String(v.variantKey || '').split('-');
    const optionValuesArr = optionNames.map((_, i) => {
      const val = parts[i] != null ? String(parts[i]) : (i === 0 ? 'Default Title' : '');
      optionValues[i].add(val);
      return { optionName: optionNames[i], name: val };
    });
    const price = repriceAUD(v.variantSugSellPrice ?? v.variantSellPrice);
    const sku = v.variantSku != null ? String(v.variantSku) : undefined;
    const input = { optionValues: optionValuesArr };
    if (price != null) {
      input.price = String(price);
    }
    if (sku) input.inventoryItem = { sku };
    input.inventoryQuantities = [{ locationId: LOCATION_ID, availableQuantity: SHOP_INVENTORY_QTY }];
    return input;
  });

  // Trim to max 3 options (Shopify hard limit) + collapse 'Title' single-option.
  ...
  const tags = [`cj-import`, `cj-pid-${pid}`];
  ...
  if (type) productInput.productType = type;
  if (d.description) productInput.descriptionHtml = d.description;
  if (opts.length) productInput.productOptions = opts;

  const images = extractImages(d).map(src => ({
    originalSource: src,
    alt: title,
    mediaContentType: 'IMAGE',
  }));

  return { productInput, gqlVariants, images };
}

// ── GraphQL helper (regular, non-bulk) ───────────────────────────────────
...
async function attachVariants(env, productId, gqlVariants, images) {
  const q = `mutation($productId: ID!, $variants: [ProductVariantsBulkInput!]!, $media: [CreateMediaInput!]) { productVariantsBulkCreate(productId: $productId, variants: $variants, strategy: REMOVE_STANDALONE_VARIANT, media: $media) { product { id } productVariants { id title price } userErrors { field message } } }`;
  const r = await gqlRaw(env, q, { productId, variants: gqlVariants, media: (images && images.length) ? images : undefined });
  const ue = (r && r.data && r.data.productVariantsBulkCreate && r.data.productVariantsBulkCreate.userErrors) || [];
  if (r && r.errors && r.errors.length) return { ok: false, error: r.errors.map(e => e.message).join('; ') };
