...
export async function onRequest(context) {
  ...
  try {
    ...
    if (body.shopifyOrderId) {
      ...
      const result = await cjFetch(env, '/shopping/order/createOrderV2', { method: 'POST', body: JSON.stringify(cjPayload) });
      return new Response(JSON.stringify(result), { headers: { 'Content-Type': 'application/json', ...corsHeaders() } });
    }
    const payload = body.products || body.line_items ? buildCjOrderFromBody(body) : body;
    const result = await cjFetch(env, '/shopping/order/createOrderV2', { method: 'POST', body: JSON.stringify(payload) });
    return new Response(JSON.stringify(result), { headers: { 'Content-Type': 'application/json', ...corsHeaders() } });
