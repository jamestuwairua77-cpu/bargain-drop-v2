...
const RUN_BUDGET_MS = 22000;   // wall-clock budget per run request (stays well under the 30s proxy timeout)
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (action === 'run') {
      ...
      st.aud0 += aud0Now;
      await shopMetaSet(env, STATE_KEY, st);
      const remaining = Math.max(0, st.total - st.done);
      return json({ ok: true, processed: idx - start, done: st.done, total: st.total, updated: st.updated, failed: st.failed, skipNoSku: st.skipNoSku, skipNoSug: st.skipNoSug, remaining, errors: st.errors.slice(0, 5) });
    }
