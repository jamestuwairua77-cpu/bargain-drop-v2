...
export async function onRequest(context) {
  ...
  try { const body = await request.json().catch(() => ({})); await appendSyncLog(env, { action: 'save-order', order: body }); return new Response(JSON.stringify({ success: true }), { headers: { 'Content-Type': 'application/json', ...corsHeaders() } }); }
