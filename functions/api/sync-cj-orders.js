...
export async function onRequest(context) {
  ...
  try { const result = await cjFetch(env, '/shopping/order/list?pageNum=1&pageSize=50'); return new Response(JSON.stringify({ success: true, total: result.data?.total || 0, orders: result.data?.list || [] }), { headers: { 'Content-Type': 'application/json', ...corsHeaders() } }); }
