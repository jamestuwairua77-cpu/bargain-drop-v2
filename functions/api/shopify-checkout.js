...
export async function onRequest(context) {
  ...
  try { const body = await request.json().catch(() => ({})); const result = await shopifyFetch(env, '/checkouts.json', { method: 'POST', body: JSON.stringify({ checkout: body }) }); return new Response(JSON.stringify(result.body), { status: result.ok ? 200 : result.status, headers: { 'Content-Type': 'application/json', ...corsHeaders() } }); }
