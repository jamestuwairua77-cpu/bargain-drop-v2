...
function renderRelatedProducts(){
  ...
  x.open('GET','/data/'+catFile,true);
  x.timeout=15000;
  x.onload=function(){
    if(x.status!==200)return;
    try{var catData=JSON.parse(x.responseText);if(!catData)return;
      // /data/{cat}.json can be either an array or {products:[...]}
      var relatedList = Array.isArray(catData) ? catData : (catData.products || []);
      if(!relatedList.length)return;
      var related=relatedList.filter(function(p){return String(p.id)!==String(pid)});
